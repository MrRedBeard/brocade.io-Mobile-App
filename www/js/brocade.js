...
class brocadeWrapper
{
    ...
    initWrapper()
    {
        ...
        this.callOutEl = this.containerEl.querySelector('.callOut');
    }

    callOut(msg, type)
    {
        this.callOutEl.innerHTML = msg;
    ...
    getListItemsData(searchTerm)
    {
            ...
            x2.ajax(
                {
                    ...
                    success: function (response, headers)
                    {
                        ...
                        if (response !== null)
                        {
                            ...
                            if (upcData !== '{}' && upcData !== '[]')
                            {
                                upcHeader = this.upcHeaderToJson(headers.toString());
                                this.barcodeItemHeader = upcHeader;
                            }
                        }
                    ...
                    error: function ()
                    {
                        this.callOut('Server Error');
                    }.bind(this)
                });            
        }
        else
        {
            upcHeader = `cache-control: max-age=0, private, must-revalidate
                connection: keep-alive
                content-type: application/json; charset=utf-8
                date: Fri, 19 Jun 2020 12:11:15 GMT
                etag: W/"5b7a350a39809d21220883b47b85c1bd"
                link: <https://www.brocade.io/api/items?page=36&query=Corn>; rel="last", <https://www.brocade.io/api/items?page=2&query=Corn>; rel="next"
                per-page: 100
                ...
                x-request-id: 7c79b148-6c82-4dc7-a44d-4f7e589beaf0
                x-runtime: 14.975202
            ...
            upcHeader = `cache-control: max-age=0, private, must-revalidate
                connection: keep-alive
                content-type: application/json; charset=utf-8
                date: Wed, 24 Jun 2020 03:00:34 GMT
                etag: W/"4f60a2b3cfae0bd1ec6f246c05b871f0"
                link: <https://www.brocade.io/api/items?page=69&query=Butter>; rel="last", <https://www.brocade.io/api/items?page=2&query=Butter>; rel="next"
                per-page: 100
                referrer-policy: strict-origin-when-cross-origin
                server: Cowboy
                strict-transport-security: max-age=31536000; includeSubDomains
                ...
                x-frame-options: SAMEORIGIN
                x-permitted-cross-domain-policies: none
                x-request-id: 4790c7f2-06f0-4b84-93d9-114e9e0850de
                x-runtime: 14.959912
                x-xss-protection: 1; mode=block`;

            upcHeader = this.upcHeaderToJson(upcHeader);

            this.barcodeItemHeader = upcHeader;

            //document.querySelector('textarea').innerHTML = JSON.stringify(upcHeader);
        ...
        if (upcData && upcData.length > 0 && upcData !== '[]')
        {
            upcData = JSON.parse(upcData);
        }
        else
        {
            this.callOut('No results found');
            this.getLookupInterface();
            return;
        }
    ...
    upcHeaderToJson(upcHeader)
    {
        while (upcHeader.includes(',') || upcHeader.includes("'") || upcHeader.includes('                '))
        {
            upcHeader = upcHeader.replace(',', ';').replace('"', '').replace('                ', '');
        }

        upcHeader = upcHeader.replace(/(\r\n|\n|\r)/gm, '\n');
        upcHeader = upcHeader.split('\n');

        let last;
        let next;

        for (var i = 0; i < upcHeader.length; i++)
        {
            if (upcHeader[i].trim() === '' || upcHeader[i] === /(\r\n|\n|\r)/gm || upcHeader[i] === "\n")
            {
                upcHeader.splice(i, 1);
                i--;
            }

            upcHeader[i] = upcHeader[i].replace(': ', '": "');
            upcHeader[i] = '"' + upcHeader[i] + '"';

            while (upcHeader[i].includes('"": ""') || upcHeader[i].includes('"","'))
            {
                upcHeader[i] = upcHeader[i].replace('"": ""', '": "');
                upcHeader[i] = upcHeader[i].replace('"","', '","');
            }

            if (upcHeader[i].includes('link'))
            {
                let tempArr = upcHeader[i].split(': ')[1].split(';');
                last = tempArr[0];
                next = tempArr[2];

                while (last.includes('"') || next.includes('"') || last.includes('<') || next.includes('<') || last.includes('>') || next.includes('>') || last.includes('  ') || next.includes('  ') || next.includes('""'))
                {
                    last = last.replace('"', '');
                    next = next.replace('"', '');
                    last = last.replace('<', '');
            ...
            if (upcHeader[i].includes('cache-control') || upcHeader[i].includes('connection') || upcHeader[i].includes('content-type') || upcHeader[i].includes('date') || upcHeader[i].includes('etag') || upcHeader[i].includes('referrer-policy') || upcHeader[i].includes('server') || upcHeader[i].includes('strict-transport-security') || upcHeader[i].includes('transfer-encoding') || upcHeader[i].includes('via') || upcHeader[i].includes('x-content-type-options') || upcHeader[i].includes('x-download-options') || upcHeader[i].includes('x-frame-options') || upcHeader[i].includes('x-permitted-cross-domain-policies') || upcHeader[i].includes('x-request-id') || upcHeader[i].includes('x-runtime') || upcHeader[i].includes('x-xss-protection') || upcHeader[i].includes('last') || upcHeader[i].includes('next'))
            {
                upcHeader.splice(i, 1);
                i--;
            }
        }
            //"per-page": "100","total": "6893","","next": "https://www.brocade.io/api/items?page=2&query=Butter","last": "https://www.brocade.io/api/items?page=69&query=Butter"}
        ...
        //document.querySelector('textarea').innerHTML = upcHeader;

        upcHeader = JSON.parse(upcHeader);
    ...
    getItem()
    {
        ...
        if (!this.currentItemUPC.includes('{') || !this.currentItemUPC.includes('}'))
        {
            this.getLookupInterface();
        }
        ...
        else
        {
            ...
            //this.currentItem = '{"error":"Product &apos;0&apos; not found"}';
        }
        ...
        if (this.currentItem && this.currentItem.length > 0)
        {
            ...
            if (this.currentItem.error && this.currentItem.error.length > 0)
            {
                this.callOut('Item not found');
                this.getLookupInterface();
                return;
            }
